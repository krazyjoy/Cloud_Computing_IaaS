...
def send_one_request(image_path):
    ...
    if response.status_code != 200:
        ...
        # filename    = image_path.split('/')[-1] # if filepath use /
        ...
        print(response.text.split(':')[1].strip())
        if correct_result == response.text.split(':')[1].strip():
...
#     image_path = os.path.join(image_folder, name)
#     image_path_list.append(image_path)
#     # image_path = image_folder + '/' + name  # change image_path
#     # image_path_list.append(image_path)
#
# with ThreadPoolExecutor(max_workers = num_max_workers) as executor:
#     executor.map(send_one_request, image_path_list)
